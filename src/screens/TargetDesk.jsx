              ...
              {!created ? (
                ...
                <div style={{ marginTop: 14, background: created.dryRun ? 'var(--gold-bg)' : 'var(--green-bg)', border: `1px solid ${created.dryRun ? 'var(--gold)' : 'var(--green)'}`, borderRadius: 11, padding: '13px 15px' }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--txt2)', lineHeight: 1.7 }}>
                    {(created.log || []).map((l, i) => <div key={i}>{l.step === 'campaign' ? '📁 Кампания' : '📄 Адсет'}{l.audience ? ` · ${l.audience}` : ''}{l.willCreate ? ` — «${l.willCreate.name || ''}»${l.willCreate.daily_budget ? `, ${Math.round(l.willCreate.daily_budget / 100)}$/дн` : ''}` : ''}{l.id ? ` ✓ id ${l.id}` : ''}{l.error ? <span style={{ color: 'var(--red)', wordBreak: 'break-word' }}> ✗ {(typeof l.error === 'string' ? l.error : (l.error.error_user_msg || l.error.message || JSON.stringify(l.error))).slice(0, 300)}</span> : ''}{(l.notes && l.notes.length) ? <span style={{ color: (l.notes.join(' ').includes('✓') && !l.notes.join(' ').includes('⚠️')) ? 'var(--green)' : 'var(--gold)' }}> · {l.notes.join('; ')}</span> : null}</div>)}
                  </div>
