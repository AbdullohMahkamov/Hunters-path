              ...
              <div className="side-nav">
                <div className="sec-dropdown-wrap" ref={secWrapRef}>
                  ...
                  <div className={'sec-dropdown' + (secOpen ? ' open' : '')} id="secDropdown">
                    ...
                    {isAdmin && <button className="menu-item" onClick={() => { setSecOpen(false); window.openMarketingModal && window.openMarketingModal() }}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M4 20L20 4M7 4H4v3M17 20h3v-3M12 12l4-1-1 4z" /></svg>{uz ? 'Marketolog (akkaunt)' : 'Маркетолог (аккаунт)'}</button>}
                    {isAdmin && <a className="menu-item" href="/target" onClick={() => setSecOpen(false)}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9" /><circle cx="12" cy="12" r="4" /><circle cx="12" cy="12" r="1" /></svg>{uz ? 'Target' : 'Таргет'}</a>}
