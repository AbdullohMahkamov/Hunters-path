...
export function renderMopTeam(_mopData) {
  ...
  const noms = []
  const bsl = best((m) => m.sold, true); if (bsl) noms.push(['🏆', mt('salesW'), bsl, bsl.sold + ' · ' + fmtSumM(bsl.revenue)])
  ...
  if (me) {
    ...
    placesCard = `<div class="mop-card">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px;">
        ${places.map(([lbl, r]) => { const col = r === 1 ? 'var(--green)' : (r <= 3 ? 'var(--gold)' : 'var(--txt2)'); return `<span style="display:inline-flex;align-items:center;gap:6px;background:var(--bg);border:1px solid var(--line2);border-radius:999px;padding:6px 12px;font-size:13px;"><b style="color:${col};font-size:15px;">№${r}</b> ${lbl}</span>` }).join('')}
      </div>
    </div>`
  }
  ...
  const topList = (icon, title, fn, higher, valFn, valColor, badges) => {
    ...
    const list = arr.map((m, i) => {
      ...
      return `<div class="mop-rank-row${isMe ? ' me' : ''}" style="padding-top:8px;padding-bottom:8px;">
        ...
        <b style="font-size:13.5px;color:${valColor || 'var(--txt)'};white-space:nowrap;">${valFn(m)}</b>
      </div>`
    }).join('')
    return `<div class="mop-card"><div class="mop-ct">${icon} ${title}</div>${list}</div>`
  }
  const topSales = topList('🏆', mt('salesW'), (m) => m.sold, true, (m) => `${m.sold} · ${fmtSumM(m.revenue)}`, 'var(--green)', true)
  const topConv = topList('🎯', mt('conv'), (m) => m.conv, true, (m) => m.conv + '%', 'var(--accent)')
