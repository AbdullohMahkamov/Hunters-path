...
const ZONES = {
  ...
  ceo: { pts: [[156, 130]], att: [156, 130] },
}
// ОБХОД CEO — непрерывный круг по всем комнатам, БЕЗ простоев.
// ⚠️ Точки НЕ на глаз: посчитаны BFS по маске проходимости room.png (порог яркости 70) с упрощением
// по прямой видимости — каждый отрезок гарантированно не режет стены (pathfinding в сцене нет).
// Формат шага: [x, y, пауза мс, флаг]. Пауза + 'flow' = короткая «сверка» со значком ✉ у агента.
// В стене Технической прорезан дверной проём (room.png, x148-154/y78-82) — CEO заходит внутрь к столу.
const CEO_LOOP = [
  [148, 78, 0], [128, 66, 1500, 'flow'], [148, 78, 0],            // Техническая — Менеджер по аналитике (через дверь)
  [156, 130, 0], [142, 126, 0], [115, 121, 0], [112, 131, 0], [115, 140, 0], [128, 174, 0],
  [128, 178, 1500, 'flow'],                                       // Аналитика — Агент по развитию
  [143, 171, 0], [143, 176, 0],
  [212, 177, 1500, 'flow'],                                       // Отдел продаж — Тренер
  [290, 178, 1500, 'flow'],                                       // Отдел продаж — Супервайзер
  [212, 177, 0], [143, 176, 0], [143, 171, 0], [112, 131, 0], [115, 121, 0], [142, 126, 0], [156, 130, 0],
]
...
export default function ScenePanel({ active = true }) {
  ...
  useEffect(() => {
    ...
    // CEO: идёт по кругу CEO_LOOP шаг за шагом и НИКОГДА не останавливается насовсем —
    // даже когда есть предложение на решении (счётчик и пузырь показываются на ходу).
    function decideCeo(a) {
      a.loopI = ((a.loopI || 0) + 1) % CEO_LOOP.length
      const s = CEO_LOOP[a.loopI]
      a.flab.textContent = 'сверка'
      goPath(a, [[s[0], s[1], s[2] || 0, s[3] || 0]])
    }
    // НЕСКОЛЬКО СЦЕНАРИЕВ движения — иначе все агенты ходят одинаково-заводным маятником.
    // Все точки берутся из зоны агента (a.pts), т.е. остаются в своей комнате и не режут стены.
    function decideAgent(a) {
      const r = Math.random()
      if (r < 0.30) goPath(a, [[...randWP(a), 1400 + Math.random() * 2600]])                          // обычный переход
      else if (r < 0.50) goPath(a, [[...a.pts[0], 5000 + Math.random() * 3500]])                       // засел за столом
      else if (r < 0.68) goPath(a, [[...randWP(a), 300], [...randWP(a), 900 + Math.random() * 800]])   // быстрый обход двух точек
      else if (r < 0.84) goPath(a, [[...randWP(a), 800], [...a.pts[0], 2200 + Math.random() * 1500]])  // сходил и вернулся
      else goPath(a, [[...randWP(a), 3000 + Math.random() * 4000]])                                    // долгая задумчивая пауза
    }
    function decide(a) {
      if (a.id === 'ceo') { decideCeo(a); return } // всегда в движении по агентам, att не используем
      if (a.waiting) { goPath(a, [[...ZONES[a.id].att, 900]]); return }
      decideAgent(a)
    }
