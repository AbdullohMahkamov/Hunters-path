...
// ЕДИНСТВЕННЫЙ длинный проход — поэтому ходим по нему, а не «сквозь» комнаты.
// Шаг: [x, y, пауза, флаг, «с кем сверяется»]. Агент из 5-го поля выходит навстречу (см. meetCeo).
const CEO_LOOP = [
  [72, 167, 500],
  [128, 167, 2000, 'flow', 'growth'],  // над Агентом по развитию
  [212, 167, 2000, 'flow', 'task'],    // над Тренером
  [290, 167, 2000, 'flow', 'mop'],     // над Супервайзером
  [330, 167, 500],
  [290, 167, 0], [212, 167, 0], [128, 167, 0],
]
...
export default function ScenePanel({ active = true }) {
  ...
  useEffect(() => {
    ...
    // Агент выходит НАВСТРЕЧУ CEO к своей точке внимания — иначе CEO «сверяется» с пустым местом,
    // пока агент по своему сценарию ушёл в другой угол.
    function meetCeo(id) {
      const b = A[id]; if (!b) return
      b.flab.textContent = 'сверка'; b.queue = []
      goPath(b, [[...ZONES[id].att, 2400, 'flow']])
    }
    function decideCeo(a) {
      a.loopI = ((a.loopI || 0) + 1) % CEO_LOOP.length
      const s = CEO_LOOP[a.loopI]
      a.flab.textContent = 'сверка'
      if (s[4]) meetCeo(s[4]) // зовём агента заранее — выйдет к моменту прихода CEO
