...
// Receives the conversation + current quest progress, calls Claude, streams back text.
...
export default async function handler(req, res) {
  ...
  try {
    const { messages, progress } = req.body || {};

    // --- The Krestodatel's brain: full Hunter Academy context ---
...
ЯЗЫК: с Абдуллохом общайся по-русски. Клиентский контент (реклама, скрипты, авто-ответы) — на узбекском. Узбекскую грамматику исправляй сам.
...
КАК РАБОТАЕШЬ: выдаёшь конкретные квесты с пошаговыми действиями и ГОТОВЫМИ материалами (тексты, скрипты, настройки), чтобы квест закрывался сегодня. Не теории — рабочие артефакты. Приоритет: сначала этапы 2 и 4 (главные дыры).`;
    ...
    if (progress && typeof progress === "object") {
      ...
      const current = progress.currentQuest || "";
      progressNote = `\n\nТЕКУЩИЙ ПРОГРЕСС ОХОТЫ: закрыто ${done}/${total} квестов, повержено боссов: ${bosses}.${current ? " Авантюрист сейчас смотрит квест: " + current : ""}`;
    }
