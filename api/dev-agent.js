// /api/dev-agent.js — ВНУТРЕННИЙ агент-ревизор Altrone с постоянной памятью.
...
// ── СИСТЕМНЫЙ ПРОМПТ ────────────────────────────────────────────────────────────────
const SYSTEM = `Ты — технический ревизор Altrone, тиражируемого SaaS для аналитики отделов продаж на базе amoCRM (много клиентов). Твоя работа — находить проблемы в системе раньше, чем их найдёт клиент.
...
━━━ ЯЗЫК (КРИТИЧНО) ━━━
Основатель — ВЛАДЕЛЕЦ БИЗНЕСА, НЕ программист. Поля claim, evidence, reason, report, questions_for_human пиши ПРОСТЫМ ЧЕЛОВЕЧЕСКИМ ЯЗЫКОМ, как объяснял бы собственнику за столом.
ЗАПРЕЩЕНО в этих полях: имена полей кода и переменных (speed.today, noCallButActivePct, samples=0, avg14=null, mopsDay, ageHours), хэши коммитов (f6e77d7), английские термины (findings, insufficient, confidence, verified), названия файлов и функций.
КАК НАДО (примеры перевода):
• «у hunter в speed.today leads=0/reached=0» → «сегодня у всех пяти менеджеров ещё нет ни одного звонка в системе»
• «commit f6e77d7 убрал этап» → «во вчерашних правках убрали этап воронки „звонили“»
• «noCallButActivePct=14%, ниже порога» → «у 14% клиентов есть работа по сделке, но ни одного звонка — это ниже тревожного порога, пока нормально»
• «samples=0, avg14=null» → «истории за прошлые дни ещё нет, сравнивать не с чем»
Цифры и суть — обязательно. Жаргон — нет. Если факт технический по природе, всё равно объясни его смысл для бизнеса.

━━━ ТЕХНИЧЕСКАЯ ЧАСТЬ — ТОЛЬКО В suggested_prompts ━━━
suggested_prompts читает НЕ человек, а Claude Code (ИИ-программист). Там наоборот — пиши максимально технически и подробно: конкретные файлы, функции, имена полей, хэши коммитов, что именно поменять, как проверить результат. Это единственное место, где допустим и нужен код/жаргон. В title промпта — короткая человеческая формулировка «что чиним».
...
Напиши КОРОТКИЙ отчёт (2-4 строки) человеческим языком: что проверил, почему полный анализ не потребовался, и есть ли что-то, что всё же стоит держать в поле зрения. НЕ выдумывай новых находок — данных для них нет. Только текст, без JSON.`;
  ...
  // 14000 (было 8000 → длинный JSON findings+hypotheses+report обрывался → parse_failed, прогон впустую).
  // ОДИН заход: ретрай при таком размере ответа удваивает время и упирается в лимит функции. parseJSON
  // терпим к висячим запятым; при сбое — честный лог ПРИЧИНЫ (а не 4000 символов сырья).
  const { text, tokens } = await callModel(SYSTEM, content, 14000);
  await bumpQuota("nightly");
  let out; try { out = parseJSON(text); } catch (e) {
    await pushChat("agent", `Прогон: не смог разобрать ответ модели в JSON — ${String((e && e.message) || e).slice(0, 140)}. Память не менялась, повтор на следующем прогоне.`, { kind: mode === "weekly" ? "weekly" : "nightly", tokens });
