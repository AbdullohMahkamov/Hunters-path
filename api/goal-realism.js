...
export function deriveLeverTasks(v) {
  ...
  // МАРКЕТИНГ: рост потока лидов до пика ёмкости команды. ВТОРИЧНО к закрытию (больше лидов при низком закрытии =
  // слив бюджета), поэтому «подготовить», а не «залить». Увеличение рекламного бюджета — решение владельца.
  if (v.levers && v.levers.budgetGain > 0) {
    const potM = Math.round(v.levers.budgetGain / 1e6);
    tasks.push({
      title: `Подготовить рост потока лидов до пика ёмкости команды`,
      why: `Команда способна обработать больше лидов, чем приходит сейчас (потенциал ~+${potM} млн выручки на пике). Проработать каналы и креативы под рост трафика. ВАЖНО: приоритет — закрытие (главный резерв); наращивать трафик при низком закрытии — сливать бюджет. Увеличение рекламного бюджета — решение владельца.`,
