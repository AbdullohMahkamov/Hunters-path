...
export default async function handler(req, res) {
  ...
  if (action === "create") {
    ...
    for (const t of splitIn) {
      ...
      const notes = [];
      if (t.type === "interest") { const ints = await resolveInterests(t.interestQueries || ["Sales", "Business", "Entrepreneurship"]); if (ints.length) { targeting.flexible_spec = [{ interests: ints }]; targeting.targeting_automation = { advantage_audience: 0 }; notes.push(`✓ интересы: ${ints.map((x) => x.name).join(", ")}`); } else notes.push("⚠️ интересы не нашлись в Meta — пока широкая по стране"); }
      if (t.type === "lookalike") { if (lookalikeId) { targeting.custom_audiences = [{ id: lookalikeId }]; targeting.targeting_automation = { advantage_audience: 0 }; notes.push("✓ Lookalike из покупателей amoCRM"); } else { notes.push("⚠️ Lookalike из amoCRM ещё не собран — нажми «Собрать Lookalike» (пока широкая)"); } }
