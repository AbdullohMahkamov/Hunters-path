...
const DEFAULT_CONFIG = {
  ...
  // ═══════════════ ДЕТЕКТОР «ЛИД БЕЗ ЕДИНОГО ЗВОНКА» — ВКЛЮЧЁН ═══════════════
  // ВНИМАНИЕ: живое значение живёт в Redis (mopagent:config); этот дефолт — лишь запасной.
  // История: 14.07.2026 был временно выключен из-за ТЕХНИЧЕСКОГО сбоя интеграции телефония↔amoCRM
  // (звонки не долетали до CRM как события/ноты — 0 из 26 в диагностике). Это была ПОЛОМКА ИНТЕГРАЦИИ,
  // НЕ поведение людей: менеджеры звонят ТОЛЬКО через подключённую к CRM телефонию, не с личных.
  // Интеграция восстановлена → детектор включён обратно 20.07.2026 (Redis: noCallReEnabledAt).
  // ЖИВОЙ ПРЕДОХРАНИТЕЛЬ (telephonyGate): даже при noCallEnabled=true детектор молчит, если данные
  // говорят, что звонки идут мимо CRM (speed.telephony.callsBypassSuspected) — защита от повтора сбоя.
  // ПЛАН (карта, п.2): заменить косвенный порог callsBypassSuspected на прямой сторож
  // «% выигранных сделок с 0 звонков» — он ловит технический обрыв телефонии сам, без человека.
  noCallEnabled: true,
  noCallDisabledAt: null,
  noCallDisabledReason: "",
  telephonyGate: true,
