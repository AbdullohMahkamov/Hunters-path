...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      // Дата снимка — по ТАШКЕНТУ (как monthStart), а НЕ по UTC. Иначе в окне 00:00–05:00 Ташкента (когда по UTC
      // ещё прошлое число) поздний sync перезаписывал бы снимок последнего дня месяца данными нового месяца (≈0),
      // и закрытие месяца читало бы ноль. С ташкентским ключом снимок последнего числа = полный итог месяца.
